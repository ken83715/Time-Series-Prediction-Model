...
for n in range(neunum): #num of neural network
    ...
    for k in range(epochs): # num of epoch
        ...
        if pasterror > error_h and math.isnan(error_h) != True:
            pasterror = error_h
            hamm.saveneu('TDCS/models/b_std_15_base_temp.pkl')
        else:
            f = open('TDCS/models/b_std_15_base_temp.pkl', 'rb')
            hamm = pickle.load(f)
            f.close()
            break
...
for n in range(neunum): #num of neural network
    ...
    for k in range(epochs): # num of epoch
        ...
        if pasterror > error and math.isnan(error) != True:
            pasterror = error
            testneu.saveneu('TDCS/models/b_std_15_base_temp2.pkl')
        else:
            f = open('TDCS/models/b_std_15_base_temp2.pkl', 'rb')
...
bestneu_h.maxd = maxd
bestneu_h.mind = mind
bestneu.saveneu('TDCS/models/b_std_15_base.pkl')
bestneu_h.saveneu('TDCS/models/b_std_15_base_h.pkl')
...
plt.title('std 1.5 Error distribution')
