...
tempmodel_path = 'TDCS/models/base_tot_temp.pkl'
model_path = 'TDCS/models/base_tdcs_tot.pkl'
model_h_path = 'TDCS/models/base_tdcs_tot_h.pkl'
...
for i in range(len(cleardata) - data_per_day * test_day * 2, len(cleardata) - input_num):
    ...
    for j in range(input_num + 1):
        if j < input_num:
            temp.append(cleardata[i + j])
            temp2.append(DATASET.roaddata[i + j])
        else:
...
plt.title('process_total_average')
...
plt.title('process_total_average Error distribution')
